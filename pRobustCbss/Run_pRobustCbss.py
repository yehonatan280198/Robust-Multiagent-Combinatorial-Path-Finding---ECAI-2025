...
class pRobustCbss:
    ...
    def __init__(self, Positions, GoalLocations, no_collision_prob, delaysProb, num_of_cols, num_of_rows, verifyAlpha, originalCheckRoot, rotate, prob):
        ...
        self.originalCheckRoot = originalCheckRoot
        self.rotate = rotate
        self.prob = prob
    ...
    def run(self):
        ...
        # self.kOptimalSequences[1] = kBestSequencing(self.Positions, self.GoalLocations, 1, self.num_of_cols, self.rotate).Solution
        self.kOptimalSequences[1] = kBestSequencingWithGLKH(self.Positions, self.GoalLocations, 1, self.num_of_cols).Solution
        ...
        LowLevelPlan(Root, self.num_of_cols, self.num_of_rows, self.Positions, list(range(len(self.Positions))), self.rotate)
        ...
        while not self.OPEN.empty():
            ...
            if not self.originalCheckRoot:
                N = self.CheckNewRoot(N)
            else:
                N = self.CheckNewRootOriginal(N)
            ...
            if self.prob:
                ...
                if verifyOriginal(N.paths, self.delaysProb, self.no_collision_prob, self.verifyAlpha):
                    return N.paths
            ...
            if self.prob:
                ...
                time, x, agent1, agent2 = self.getlConflictOriginal(N)

                A1 = self.GenChild(N, negConst(agent1, x, time))
                A2 = self.GenChild(N, negConst(agent2, x, time))
    ...
    def CheckNewRoot(self, N):
        ...
        LowLevelPlan(newRoot, self.num_of_cols, self.num_of_rows, self.Positions, list(range(len(self.Positions))), self.rotate)
    ...
    def GenChild(self, N, NewCons):
        ...
        if isinstance(NewCons, negConst):
            ...
            LowLevelPlan(A, self.num_of_cols, self.num_of_rows, self.Positions, [NewCons.agent], self.rotate)
