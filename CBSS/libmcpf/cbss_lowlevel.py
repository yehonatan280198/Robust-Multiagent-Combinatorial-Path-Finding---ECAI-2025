...
import numpy as np
from CBSS.libmcpf import common as cm
import time
import sys
...
class SippState:
  ...
  """
  def __init__(self, sid, loc, gval, t, tb):
    """
    """
    self.id = sid
  ...
  def __str__(self):
    return "{"+str(self.id)+","+str(self.loc)+",g("+str(self.g)+"),itv("+str(self.t)+","+str(self.tb)+")}" 
...
class SIPP:
  ...
  """
  def __init__(self, grids, sx, sy, gx, gy, t0, ignore_goal_cstr, w=1.0, eps=0.0, action_set_x = [-1,0,1,0], action_set_y = [0,-1,0,1]):
    """
    """
    self.grids = grids
...
def Test1():
  ...
  """
  grids = np.zeros((3,3))
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 0, 1.0, 0.0, 10.0, [(1,1)], []) # node cstr
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 0, 1.0, 0.0, 10.0, [], [(0,1,0)]) # edge cstr
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 0, 1.0, 0.0, 10.0, [(1,1)], [(1,2,2)]) # both node and edge cstr
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 0, 1.0, 0.0, 10.0, [(1,1), (2,6)], [(1,2,2)]) # node cstr at destination.
  sol_path, stats = RunSipp(grids, 0,0, 2,0, 5, 1.0, 0.0, 10.0, [(1,1), (2,6)], [(1,2,2)]) # non-zero starting time step.
  print(sol_path)
  print(stats)
  return
