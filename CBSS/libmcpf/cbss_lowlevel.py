...
class SippGridSpace:
  ...
  def GetSuccSafeItv_ml(self, nid, nnid, tlb, tub):
    """
    return a list of safe intervals, tlist, locate at nnid that are reachable/successors from nid with safe interval [tlb, tub].

    update0624:t0 is arrival time at node nnid, so closed interval [tlb,t0-1] is waiting time at node nid, so t0-1 must be less than or equal to tub
    """
    out = list()
    unsafe_tpoint = list()
    # print(" nid ", nid, " nnid ", nnid, " cstrs : ", self.node_cstrs)
    ...
    if len(unsafe_tpoint) > 0:
      for idx in range(len(unsafe_tpoint)):
        ...
        if tk <= t0:
          if tk == t0:
            t0 = tk + 1  ## earliest possible arrival time should be shift forwards.
            if t0-1 > tub:  # everytime when t0 changes, need to check
              break
          continue
        ## now tk >= t0 + 1
        if tk <= tub + 1:
