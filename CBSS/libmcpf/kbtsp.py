...
class KBestMTSP:
  ...
  def ComputeNextBest(self, tlimit, num_vertices):
    ...
    """
    self.num_vertices = num_vertices                                    # Set the total number of vertices
    self.tstart = time.perf_counter()                                   # Record the current time to measure the time limit
    if len(self.kbest_node) == 0:                                       # Check if this is the first time computing
      self._Init()                                                      # Initialize the first best solution
    else:
