...
from CBSS.pytspbridge import tf_mtsp, tsp_wrapper
...
class SeqMSMP(object):
  ...
  """
  def __init__(self, grid, Vo, Vt, configs, delays):
    super(SeqMSMP, self).__init__()
    self.grid = grid
    self.Vo = Vo
    self.Vt = Vt
    self.V = self.Vo + self.Vt
    self.N = len(self.Vo)
    self.M = len(self.Vt)
    ...
    self.setOe = set()
    self.delays = delays
    self.assignGoals = None
    return

  def InitMat(self):
    self.spMat = cm.getTargetGraph(self.grid,self.Vo,self.Vt, self.delays) # target graph, fully connected.
  ...
  def Solve(self):
    ...
    res = tsp_wrapper.invoke_lkh(self.tsp_exe, problem_str, self.delays)
