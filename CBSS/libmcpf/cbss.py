...
import CBSS.libmcpf.kbtsp as kb
import CBSS.libmcpf.cbss_lowlevel as sipp_ml
import CBSS.libmcpf.common as cm
import copy
import time
import numpy as np
import sys
        ...
        while ix < len(self.paths[i][1]) - 1:
            for jx in range(len(self.paths[j][1]) - 1):
                ...
                if ivb == jvb:  # vertex conflict at ivb (=jvb)
                    return [CbsConstraint(i, ivb, ivb, t_lb + 1, t_lb + 1, j, 1),
                            CbsConstraint(j, jvb, jvb, t_lb + 1, t_lb + 1, i, 1)]  # t_ub might be inf?
                    # use min(itb,jtb) to avoid infinity
                if (ivb == jva) and (iva == jvb):  # swap location
                    return [CbsConstraint(i, iva, ivb, t_lb, t_lb + 1, j, 2),
                            CbsConstraint(j, jva, jvb, t_lb, t_lb + 1, i, 2)]
            ix = ix + 1
        return []
        ...
        while ix < len(self.paths[i][1]) - 1:
            for jx in range(len(self.paths[j][1]) - 1):
                ...
                if len(common_elements) != 0:  # vertex conflict at ivb (=jvb)
                    return [CbsConstraint(i, common_elements[0], common_elements[0], t_lb + 1, t_lb + 1, j, 1),
                            CbsConstraint(j, common_elements[0], common_elements[0], t_lb + 1, t_lb + 1, i,
                                          1)]  # t_ub might be inf?
                    # use min(itb,jtb) to avoid infinity
                if (ivb == jva) and (iva == jvb):  # swap location
                    return [CbsConstraint(i, iva, ivb, t_lb, t_lb + 1, j, 2),
...
class CbssFramework:
    def __init__(self, mtsp_solver, grids, starts, goals, ac_dict, configs, delays):
        ...
        self.starts = starts
        self.goals = goals
        self.total_num_nodes = len(starts) + len(goals)
    ...
    def _IfNewRoot(self, curr_node):
        ...
        if DEBUG_CBSS:
            print("### CBSS _IfNewRoot 2nd phase, input cost = ", cval, " eps_cost = ", self.eps_cost, " next_cost = ",
                  (1 + self.eps) * self.next_seq.cost)
    ...
    def _GenNewRoot_multi_ml(self):
        ...
        nid = self.node_id_gen                              # Get the current node ID
        self.nodes[nid] = self._GenCbssNode(nid)            # Create a new CBSS node and add it to the nodes dictionary
        self.node_id_gen = self.node_id_gen + 1             # Increment the node ID generator for the next node
        self.root_set.add(nid)                              # Add the current node ID to the set of root nodes
        ...
        if not self.next_seq:                                                                           # Check if there is no next sequence available
            if (nid == 1):  # init
                ...
                if not flag:                                                                            # Check if no feasible sequence was found or if time ran out
                    ...
                    sys.exit("[ERROR]")
                self.root_seq_dict[nid] = self.kbtsp.GetKthBestSol()  # assign seq data to root node.   # Assign the computed sequence data to the root node
            else:
    ...
    def Lsearch_ml(self, nid, ri):
        ...
        res_path, sipp_stats = sipp_ml.RunSipp_ml(self.grids, gseq, t0, ignore_goal_cstr, 3.0, 0.0, tlimit, ncs,
                                                  ecs)  # note the t0 here!
    ...
    def Search_ml(self):
        ...
        output_res = [int(len(self.closed_set)), float(best_g_value), int(0), int(self.open_list.size()), \
                      int(self.num_closed_low_level_states), int(search_success),
                      float(time.perf_counter() - self.tstart), \
                      int(self.kbtsp.GetTotalCalls()), float(self.kbtsp.GetTotalTime()), int(len(self.root_set))]
